...
def main(args):
    ...
    if len(args) != 3 and len(args) != 5: 
        ...
        sys.exit(1)
    logging.basicConfig(format='%(asctime)s : %(levelname)s : %(message)s',level=logging.DEBUG,filename="download/log_ver2_0821.txt")
    global password
    #global pagetocrawl
    global idforUser
    global idforArticle
    ...
    url="https://www.ptt.cc/bbs/Gossiping/index36725.html" 
    nexturl=""
...
def download(articles,res): #download the content, choose what attr will be crawled
    ...
    if firsttimeruncode == 0:
        ...
        if len(latesttime)>30: #avoid wrong time format
    ...
    for article in articles:
        ...
        if check_new_author == 1:
            Author['Article'] = insert_article
            collection.insert(Author)
        else:
            print_log('Article id :' + str(article_id) + 'finished')
            collection.update_one({
                'id': author_id, "Article.Articlename":title_value
                },{
                    "$set": {
                            "Article.$.Boo": insert_article["Boo"],
                            "Article.$.Push": insert_article["Push"]
                        }
            })

...
inter_gate = 15
top30_len_dic = []
for groups in db.finalGroup.find({"date":date,"day_range":day_range,"official":official,"inter_gate":inter_gate}):
	if groups["group_id"] != 0:
		N+=groups["top30_len"]
		top30_len_dic.append((groups["group_id"],groups["top30_len"]))
top30_len_dic = dict(top30_len_dic)
print(N)
for word in db.Term.find({"date":date,"day_range":day_range,"official":official,"inter_gate":inter_gate}):
	df_total=0
	df_list=[]
